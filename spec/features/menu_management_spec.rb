...
RSpec.describe 'Menu Management', type: :feature do
  before(:each) do
    ...
    @section = Section.create!(label: 'Classic Pizzas', description: 'Traditional Italian pizzas with a variety of toppings.')
    @item = Item.create!(label: 'Margherita Pizza', description: 'Classic pizza with tomatoes, mozzarella cheese, and fresh basil.', price: 800, type: 'Product')
    ...
    @modifier = Modifier.create!(item: @modifier_item, modifier_group: @modifier_group, display_order: 1, default_quantity: 1, price_override: 0)
